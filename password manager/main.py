...
#set current and max date to the date of today
ui.add_creationDate_dateEdit.setDate(QDate(date.today()))
ui.add_creationDate_dateEdit.setMaximumDate(QDate(date.today()))
...
ui.add_telNb_lineEdit.textChanged.connect(clear_add_feedback)
...
#--------------Settings---------------#
from scripts.Dialogs import password_generator_dialog
...
ui.change_password_open_password_generator_pushButton.clicked.connect(password_generator_dialog.run)
#Password remember----------->
...
#about----------------->
from scripts.Dialogs import send_feedback_dialog
...
def open_feedback():
    send_feedback_dialog.run()

ui.about_commandLinkButton.clicked.connect(about_show)
ui.give_feedback_pushButton.clicked.connect(open_feedback)
